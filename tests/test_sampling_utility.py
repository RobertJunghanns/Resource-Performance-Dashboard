...
class TestSamplingUtilityFunctions(unittest.TestCase):
    ...
    def test_get_n_case_ids_less(self):
        sampled_ids = activity_duration_estimation.get_n_case_ids(self.case_ids, 3)
        self.assertEqual(len(sampled_ids), 3)

    def test_get_n_case_ids_more(self):
        sampled_ids = activity_duration_estimation.get_n_case_ids(self.case_ids, len(self.case_ids))
        self.assertEqual(len(sampled_ids), len(self.case_ids))

    def test_get_n_events_less(self):
        sampled_events = activity_duration_estimation.get_n_events(self.event_log, 5)
    ...
    def test_get_n_events_more(self):
        ...
        sampled_events = activity_duration_estimation.get_n_events(filtered_event_log, len(filtered_event_log))
        self.assertEqual(len(sampled_events), len(filtered_event_log))

    def test_get_trace(self):
        sampled_trace = activity_duration_estimation.get_trace(self.event_log, '173691')
    ...
    def test_group_equal_timestamp_events_1(self):
        trace = activity_duration_estimation.get_trace(self.event_log, '173691')
